...
public class JukeboxListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (holdingCustomDisc) {
            ...
            if (jukeboxHasCustom) ejectCustomDisc(block, player);
            else { Jukebox jb = (Jukebox) block.getState(); if (jb.isPlaying()) { jb.stopPlaying(); jb.update(); } }
            ItemStack discCopy = hand.clone(); discCopy.setAmount(1);
            if (player.getGameMode() != GameMode.CREATIVE) hand.setAmount(hand.getAmount() - 1);
            Jukebox jb = (Jukebox) block.getState();
            jb.setRecord(new ItemStack(Material.MUSIC_DISC_11)); jb.update();
            ...
            player.sendMessage(ChatColor.GREEN + "♪ " + discCopy.getItemMeta().getDisplayName() + ChatColor.GREEN + " ♪");
    ...
    private void ejectCustomDisc(Block block, Player player) {
        ...
        Jukebox jb = (Jukebox) block.getState();
        jb.stopPlaying(); jb.setRecord(new ItemStack(Material.AIR)); jb.update();
